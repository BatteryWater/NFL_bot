...
class Player:
    ...
    def GetInfo(self, soup):
        ...
        self.Weight = soup.find('span', {'itemprop':'weight'}).text
        ...
        self.DOB = soup.find('span', {'itemprop':'birthDate'}).text
        self.College = soup.find('span', {'itemprop':'fig'})
...
def response_bs_pfr(player, position):
    ...
    if players is None:
            ...
            responsePlayers.append(Player(info))
            #print(p)
    else:
            playerUrlList = []
            ...
            if len(playerUrlList) != 0:    
                for playerUrl in playerUrlList:
                    ...
                    responsePlayers.append(Player(player))
            else:
                print("No matches")
                #print(info)
...
def run_bot(r, comments_replied_to):
    try:
        ...
        for comment in r.subreddit('test').comments(limit=250):
            ...
            if(len(match) > 0) and comment.id not in comments_replied_to and comment.author != config.username:
                if len(match > 1):
                    comment.reply(response_bs_pfr(match[0], match[1]))
                else:
                    comment.reply(response_bs_pfr(match[0], None))
